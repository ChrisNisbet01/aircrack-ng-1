...
{
    sort_context->sort_context.sort_method = ap_sort_method_assign(SORT_BY_NOTHING);
...
#define ONE_MIN (60)
    ...
    if (lopt.interactive_mode > 0)
    {
        ...
        if (pthread_create(&lopt.input_tid, NULL, &input_thread, &lopt.input_thread_pipe[1])
                   != 0)
        {
