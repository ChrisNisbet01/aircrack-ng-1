        ...
        || h80211[0] == IEEE80211_FC0_SUBTYPE_PROBE_RESP)
    {
        ...
        while (p < data_end)
        {
            ...
            if (p[0] == 0xbf && p[1] >= 12)
            {
                 ...
                 * 0x18 & 1 == 0 doesn't it?
                 */
                ap_cur->ac_channel.mu_mimo = (p[4] & 0x18) & 1;
    ...
    int const if_num)
{
    ...
    if (options->freqoption)
	{
		...
		for (i = 1; i < if_num; i++)
		{
			snprintf(buffer, sizeof(buffer), ",%4d", interface_data[i].current_frequency);
	...
	else /* Must be channel option. */
	{
		...
		for (i = 1; i < if_num; i++)
		{
			...
			snprintf(buffer, sizeof(buffer) - 1, ",%2d", options->interface_data[i].current_channel));
			strlcat(strbuf, buffer, sizeof(strbuf));
		}
	}
    ...
    if (strlen(options->message) > 0)
	{
		...
		strlcat(strbuf, options->message, sizeof strbuf - strlen(strbuf) - 1);
	}
...
#undef T
                ...
                if (options->show_manufacturer)
				{
                    if (options->maxsize_essid_seen <= (u_int)(len - essid_len))
                    {
                        options->maxsize_essid_seen
